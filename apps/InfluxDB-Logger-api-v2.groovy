 ...
 *   2020-01-29 Brandon Soto    Support web api for InfluxDB v2.0
 *****************************************************************************************************************/
definition(
...
preferences {
    ...
    section ("InfluxDB Database:") {
        ...
        input "prefDatabasePort", "text", title: "Port", required: false
        input "prefDatabaseName", "text", title: "Database/Bucket Name", defaultValue: "Hubitat", required: true
        input "prefDatabaseOrg", "text", title: "Organization", required: true
        input "prefDatabaseToken", "text", title: "API Token", required: true
...
def handleEvent(evt) {
    ...
    def data = "${measurement},deviceId=${deviceId},deviceName=${deviceName}" //TODO: reenable. ,groupId=${groupId},groupName=${groupName},hubId=${hubId},hubName=${hubName},locationId=${locationId},locationName=${locationName}"
...
def queueToInfluxDb(data) {
    ...
    long timeNow = (new Date().time) // Time is in milliseconds
    data += " ${timeNow}"
...
def postToInfluxDB(data) {
    ...
    try {
        ...
        if (state.databaseToken) {
            ...
            */
            def hostStr = state.databasePort ? "${state.databaseHost}:${state.databasePort}" : "${state.databaseHost}"
            def dataStr = "mem,host=host1 used_percent=23.43234543 1580442405817"
            postParams = [
                uri: "http://${hostStr}",
                path: "/api/v2/write?org=${state.databaseOrg}&bucket=${state.databaseName}&precision=ms",
                // headers: ["Authorization", encodeCredentialsToken(state.databaseToken)],
                headers: ["Authorization", "Token ${state.databaseToken}"],
        ...
        logger("postToInfluxDB(): postParams=${postParams}", "debug")
        asynchttpPost(null, postParams)
    } catch (e) {
...
/**
 *  encodeCredentialsToken()
 *
 *  Encode credentials for HTTP Token authentication.
 **/
private encodeCredentialsToken(token) {
    def rawString = "Token ${token}"
    return rawString.bytes.encodeBase64().toString()
}
